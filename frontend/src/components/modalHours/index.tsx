...
export function ModalHours() {
    ...
    useEffect(() => {
        if (modalState.name === 'isSelectHoursOpen') {
            ...
            setTimeout(() => {
                hourRef.current?.scrollToIndex({ index: hourIndex >= 0 ? hourIndex : 8, animated: false });
                minuteRef.current?.scrollToIndex({ index: minuteIndex >= 0 ? minuteIndex : 0, animated: false });
            }, 100); // timeout garante que o componente esteja montado
        }
    }, [modalState.name]);
    ...
    return (
        <Modal visible={modalState.name === 'isSelectHoursOpen'} transparent animationType="fade">
