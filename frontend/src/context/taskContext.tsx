...
type dataProps = {
  ...
  selectedFilterCategory: CategoryProps | undefined;
...
interface TaskContextProps {
  ...
  setModalState: React.Dispatch<
    React.SetStateAction<{ name: ModalProps; data?: any }>
  >;
  ...
  fetchTask: () => void;
  fetchTaskBySearch: (item?: string, category?: string) => void;
  ...
  handleAddAnnotation: (data: FormData, handleBackToTask?: () => void) => void;
  ...
  handleAddMemberAnnotation: (
    id: string,
    members: membersProps[],
    handleBackToTask?: () => void
  ) => void;
  handleAddAnnotationByGroup: (
  ...
  fetchGroup: () => void;
  removeGroup: (group: string | undefined, id?: string) => void;
...
export function TaskContextProvider({ children }: TaskContextProviderProps) {
  const [data, setData] = useState({
    ...
    selectedFilterCategory: undefined as CategoryProps | undefined,
  ...
  async function getNameUser(userId: string) {
    try {
      ...
      setData((prevState) => ({
        ...prevState,
        createUserAnnotationOurGroup: response.data,
      }));
    } catch (error: any) {
      console.log(
        "Erro ao conectar com o servidor:",
  ...
  async function deleteUser() {
    try {
      Alert.alert(
        "Ecluir",
        `Deseja realmente excluir sua conta ${data.user?.name}?`,
        [
          {
            text: "Não",
            style: "cancel",
  ...
  async function deslogar() {
     try {
      ...
      setData((prevState) => ({
        ...
        createUserAnnotationOurGroup: undefined,
        userExists: "",
      ...
      setData((prevState) => ({ ...prevState, token: "", logado: false, user: null }));
    }
  }
  ...
  async function removeCategory(category: string, id?: string) {
    try {
      ...
      Alert.alert(
        "Remover categoria",
        "Não foi possivel remover essa categoria."
      );
      console.log("Erro desconhecido:", error);
  ...
  async function handleUpdateSubCategory(
    ...
    try {
      await api.put(`/sub-category/update/${id}`, {
        subCategory: name,
        categoryName,
        icon,
        color,
  ...
  async function fetchTaskBySearch(item?: string, category?: string) {
    try {
      if (data.user) {
        const response = await api.get(`/task/fetch?i=${item}&c=${category}`);
  ...
  async function handleUpdateTask({
    ...
    try {
      ...
      if (rest.name) {
        ...
        updatePayload.name = task.name;
      }
      if (rest.category) {
        updatePayload.category = rest.category;
      } else {
        updatePayload.category = task.category;
      }
      if (rest.priority) updatePayload.priority = rest.priority;
      if (rest.date) {
        updatePayload.date = new Date(rest.date).toISOString();
      } else {
        updatePayload.date = new Date(task.date).toISOString();
      }
  ...
  async function handleSubTaskRemove(
    taskId: string,
    subTask: string,
    subTaskId?: string
  ) {
      ...
      Alert.alert("Remover", `Remover a tarefa ${subTask}?`, [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              await api.delete(
                `/task/deleteSubTask/${taskId}?sub=${subTaskId}`
              );
  ...
  async function handleAddAnnotationByGroup(
    annotationId: string | undefined,
    groupId: string
  ) {
    try {
      await api.patch(`/annotation/update/${annotationId}/groups/${groupId}`);
  ...
  async function fetchAnnotationBySearch(item: string) {
    try {
      if (data.user) {
        const response = await api.get(`/annotation/fetchByUser?q=${item}`);

        setData((prevState) => ({
          ...prevState,
          annotationSearch: response.data,
        }));
      }
    } catch (error) {
      console.log("Erro desconhecido:", error);
  ...
  async function handleAddMemberAnnotation(
    id: string,
    members: membersProps[],
    handleBackToTask?: () => void
  ) {
  ...
  async function handleAddMemberGroup(id: string, members: membersProps[]) {
    try {
      ...
      if (error.response) {
        ...
        Alert.alert(
          "Novo Membro",
          "Não foi possível adicionar novo membro nesse grupo"
        );
      }
    }
  }
  ...
  async function handleRemoveMemberGroup(groupId: string, membersId: string[]) {
    try {
      ...
      if (error.response) {
        ...
        Alert.alert(
          "Remover Membro",
          "Não foi possível remover membro desse grupo"
        );
      }
    }
  }

  async function removeGroup(group: string | undefined, id?: string) {
    try {
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              data.annotationById?.attachments?.filter(
