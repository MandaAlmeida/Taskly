...
import { ModalSelectGroup } from "@/components/modalCreateGroup";
...
export function AddAnnotations() {
  ...
  const [content, setContent] = useState<contentProps[]>(
    annotation?.content ?? [{ type: "text", value: "" }]
  );
  ...
  function saveNote() {
    ...
    if (data.selectedCategory)
      formData.append("category", data.selectedCategory._id);
    ...
    otherFiles.forEach((block, index) => {
      ...
      const alreadyAttached = annotation?.attachments?.some(
        (att) => att.url === fileName
      );
    ...
    if (annotation) {
      ...
      handleAddAnnotation(formData, handleBackToAnnotation);
    }
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView>
        ...
        {content.map((block, index) => {
          ...
          if (block.type === "image") {
            ...
            const imageUri =
              foundImage?.url ??
              (typeof block.value === "string" ? block.value : null);
      ...
      <View style={styles.containerButton}>
        ...
        <TouchableOpacity
          style={styles.buttonSelect}
          onPress={() => setModalState({ name: "isCreateMemberOpen" })}
        >
        ...
        <TouchableOpacity
          style={styles.buttonSelect}
          onPress={() => setModalState({ name: "isSelectGroupOpen" })}
        >
      ...
      <View style={{ marginTop: 20 }}>
        <Button
          text={annotation ? "Editar Anotação" : "Criar Anotação"}
          onPress={saveNote}
          disabled={isDisabled}
